...
class YOLOv5Trainer(ClientTrainer):
    ...
    def set_model_params(self, model_parameters):
        ...
        recv_size = self.calculate_model_size()
        self.model.load_state_dict(model_parameters)
        logging.info(f"Received model size: {recv_size:.2f} MB")
        self.received_model_size = recv_size  # Store for logging
    ...
    def train(self, train_data, device, args):
        ...
        if self.round_idx == args.comm_round - 1:
        ...
        with lock:
            with p.open("a", newline="", encoding="utf-8") as f:
                writer = csv.writer(f)
                if self.round_idx == 0:
                    writer.writerow(["type", "client_id", "role", "round_id", 
                                    "comm_round", "round_loss", "round_duration", "lr",
                                    "num_examples", "worker_num", "recv_model_size", "sent_model_size"])
                sent_size = self.calculate_model_size()
                writer.writerow(["train", self.id, args.role, self.round_idx, 
                                args.comm_round, float(epoch_loss[-1, :].sum()),
                                time.time() - self.round_start_time, args.lr,
                                len(train_data.dataset), args.worker_num,
                                self.received_model_size, sent_size])
                logging.info(f"Trainer train {self.id} round {self.round_idx} log saved")
    ...
    def val(self, model, train_data, device, args):
        ...
        with lock:
            with p.open("a", newline="", encoding="utf-8") as f:
                ...
                if self.round_idx == 0:
                    writer.writerow(["type", "client_id", "role", "round_id",
                                    "mAP", "precision", "recall", "model_size"])
                current_size = self.calculate_model_size()
                writer.writerow(["val", self.id, args.role, self.round_idx,
                                map, mp, mr, current_size])
                logging.info(f"Trainer val {self.id} round {self.round_idx} log saved")
